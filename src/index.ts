...
let axesHelper = new AxesHelper(30);
...
function createTrajectory(data: {p, t}[], colorHex){
    const material = new LineBasicMaterial({
        color: new Color().set(colorHex)
    });
    material.onBeforeCompile = patchShader
    ...
    for (let i = 0; i < data.length-1; i++) {
        const v1 = data[i]
        const v2 = data[i+1]
        positions[i * s] = v1.p.x;
        positions[i * s + 1] = v1.p.y;
        positions[i * s + 2] = v1.p.z;
        positions[i * s + 3] = v2.p.x;
        positions[i * s + 4] = v2.p.y;
        positions[i * s + 5] = v2.p.z;
    }
    const times = data.map(p => [p.t, p.t]).flat();
    const geometry = new BufferGeometry();
    geometry.setAttribute('position', new BufferAttribute(positions, 3));
